...
async function proxyFetch(req,target,node){
  ...
  headers.set("Via","bepichon"); if(node) headers.set("X-Node",node);
...
function uiPage(env){
  ...
  <script>function go(){let u=document.getElementById('u').value;if(!u) return;if(!u.startsWith('http')) u='http://'+u;document.getElementById('fr').src='?url='+encodeURIComponent(u)+'&t=${token}';}</script></body></html>`;
